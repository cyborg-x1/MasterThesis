...
\chapter{SignRecognition}
 ...
 depth and camera info messages (see in listing \vref{lst:basic}) was copied from the nodelet being responsible 
 for point cloud creation. 
 
 The function imageCb is the callback function where all the image processing is done.
\begin{lstlisting}[caption={Code of the first Bluring Function}\label{lst:basic},language=c++]
...
\begin{lstlisting}[caption={Launchfile for }\label{lst:launch},language=xml]
<launch>
...
%TODO ---- An example result to this algorithm can be seen in figure \vref{figure:MatchingSigns}.
